<?php 
  if(!empty($_SESSION["user"]) && $_SESSION["user"]["user_is_admin"]==true){
    $maxId = $PDOconn->query("SELECT MAX(polis_vodiy_id) FROM polis_vodiy")->fetch();

    $query = "SELECT company_id, company_name FROM company;";
    
    if(isset($_POST["company_name"]) && $_POST["company_name"] != "none" &&
    isset($_POST["polis_vodiy_termin"]) && !empty($_POST["polis_vodiy_termin"])){
        $query2 = $PDOconn->prepare("INSERT INTO polis_vodiy VALUES(?,?,?,?);");
        $query2->execute(array($maxId["max"]+1,$_POST["polis_vodiy_termin"], $_POST["company_name"], $_SESSION["user"]["user_id"]));
        header("Location: index.php?action=all_vodiy_polis&success_add=1");
    }
?>
...
<div class="register-form-container">
    ...
    <form action=<?php echo "index.php?action=add_polis_vodiy"?> method="POST">
        <div class="form-fields">
            ...
            <div class="form-field">
                ...
                <input type="date" min=<?php echo date('Y-m-d'); ?> name="polis_vodiy_termin" class="input" name="polis_vodiy_termin" placeholder="Виберіть доки поліс дійсний" value=<?php 
                if(isset($_POST["polis_vodiy_termin"]) && !empty($_POST["polis_vodiy_termin"])){
                    echo $_POST["polis_vodiy_termin"];
                }
                ?>>
                <?php 
                if(isset($_POST["polis_vodiy_termin"]) && empty($_POST["polis_vodiy_termin"])){
