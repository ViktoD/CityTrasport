...
$start=0;
$page=1;
$countTzPolis = $PDOconn->query("SELECT COUNT(*) FROM polis_tz;")->fetch();
$numberPages = (int)$countTzPolis["count"]/20;
if($countTzPolis["count"]%20 != 0){
    $numberPages++;
}
...
if(isset($_GET["page"]) && $_GET["page"] <= $numberPages && $_GET["page"] >= 1){
    $page = $_GET["page"];
    $start = 20*($page-1);
    $query1 = "SELECT polis_tz_id, polis_tz_termin, company_name, user_surname, user_name
    FROM (polis_tz INNER JOIN company ON polis_tz_company=company_id)
    INNER JOIN users ON polis_tz_user_id=user_id
    ORDER BY polis_tz_termin DESC
...
?>
<button class="add"><a href="index.php?action=add_polis_tz">Додати новий страховий поліс для транспорту</a></button>
    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td>
                    <button class="view"><a href=<?php echo "index.php?action=polis_tz&polis_tz_id=".$object["polis_tz_id"];?>>Переглянути</a></button>
                </td>
                <td>
                    <button class="edit"><a href=<?php echo "index.php?action=edit_polis_tz&polis_tz_id=".$object["polis_tz_id"];?>>Редагувати</a></button>
                </td>
                <td>
                    <button class="delete"><a href=<?php echo "index.php?action=delete_polis_tz&polis_tz_id=".$object["polis_tz_id"];?> onclick="return confirm('Ви справді хочете інформацію про страховий поліс?')">Видалити</a></button>
                </td>
            </tr>
                <?php
...
<?php for($i=1; $i<=$numberPages; $i++){
    ?>
        <button class="paginate"><a href=<?php echo "index.php?action=all_tz_polis&page=".$i;?>><?php echo $i; ?></a></button>
    <?php
} ?>
