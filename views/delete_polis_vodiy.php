...
if(!empty($_SESSION["user"])&&$_SESSION["user"]["user_is_admin"]==true){
    if(isset($_GET["polis_vodiy_id"])){
        $query = $PDOconn->prepare("DELETE FROM polis_vodiy WHERE polis_vodiy_id = ?;");
        $query->execute(array($_GET["polis_vodiy_id"]));

        header("Location: index.php?action=all_vodiy_polis&success_delete=1");
    }
}
else{
    ?>
